...
document.getElementById("btnReportarFalla").addEventListener("click", async function () {
    const form = document.getElementById("formFallaTecnica");
    ...
    try {
        ...
        if (response.ok) {
            mensajeRespuesta.innerHTML = `<div class="alert alert-success">¡Falla reportada con éxito!</div>`;
        } else {
            mensajeRespuesta.innerHTML = `<div class="alert alert-danger">Error: ${result.message}</div>`;
        }
    } catch (error) {
        document.getElementById("mensajeRespuesta").innerHTML = `<div class="alert alert-danger">Ocurrió un error inesperado.</div>`;
    }
});
